...
public sealed partial class PlayerIsWerewolfChecker : IDialogueTopicProcessor {
    [GeneratedRegex("(?:if|when)(?: player| PC)?(?: is)?( not)?(?: a)? werewolf", RegexOptions.IgnoreCase)]
    private static partial Regex PlayerWerewolfRegex { get; }
    ...
    public bool CheckNote(DialogueTopicInfo topicInfo, Note note) {
        var match = PlayerWerewolfRegex.Match(note.Text);
        if (match.Success) {
